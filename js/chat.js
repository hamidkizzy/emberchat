document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function renderConvList() {
    ...
    convs = convs.filter((c) => c.user.name.toLowerCase().includes(filter) || c.user.email.includes(filter));

    convList.innerHTML = '';

    if (!convs.length) {
      const others = await EmberDB.listOtherUsers();
      const emptyMsg = document.createElement('div');
      emptyMsg.className = 'conv-empty';
      emptyMsg.textContent = others.length
        ? 'No matches.'
        : 'No one else has joined yet. Invite someone to get the conversation started.';
